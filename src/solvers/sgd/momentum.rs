...
/// [1]: ./index.html
pub struct Momentum<SolverB: IBackend + IBlas<f32>> {
    /// The gradient update from the previous iteration for each blob.
    history: Vec<ArcLock<HeapTensor>>,
    /// The backend used for computing the gradient.
    backend: Rc<SolverB>,
}

impl<SolverB: IBackend + IBlas<f32>> Momentum<SolverB> {
    ...
    fn init<B: IBackend + INn<f32>>(&mut self, net: &Network<B>) {
        self.history = Vec::with_capacity(net.learnable_weights().len());

        for weight_blob in net.learnable_weights() {
            let shape = weight_blob.read().unwrap().shape().clone();
            let history_blob = Arc::new(RwLock::new(SharedTensor::new(self.backend.device(), &shape).unwrap()));
            self.history.push(history_blob);
        }
    }
}

impl<B: IBackend + IBlas<f32>, NetB: IBackend + INn<f32>> SGDSolver<B, NetB> for Momentum<B> {
    fn compute_update_value(&mut self,
        ...
        let _ = IBlasAxpby::<f32>::axpby_plain(ISolver::<B, NetB>::backend(self),
                                               &lr_shared,
                                               weight_blob.read().unwrap().diff(),
                                               &momentum_shared,
                                               &mut history_blob.write().unwrap());

        let _ = ISolver::<B, NetB>::backend(self).copy_plain(
            &history_blob.read().unwrap(), weight_blob.write().unwrap().mut_diff());
    }
}
